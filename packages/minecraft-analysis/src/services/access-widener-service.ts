...
import { AccessWidenerParseError } from '../utils/errors';
import { logger } from '../utils/logger';
import { getDecompiledPath } from '../utils/paths';
...
export class AccessWidenerService {
  ...
  generateAccessWidener(
    ...
    namespace: MappingType = 'mojmap',
  ): string {
    const lines: string[] = [
      `accessWidener v2 ${namespace === 'yarn' || namespace === 'mojmap' ? 'named' : namespace}`,
      '',
    ];
