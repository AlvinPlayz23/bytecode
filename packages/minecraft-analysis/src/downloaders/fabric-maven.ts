...
import { getDownloadedMappingJarPath, paths } from '../utils/paths';
import { downloadFile, fetchText } from './http-client';
...
export class FabricMavenClient {
  ...
  async downloadIntermediaryMappings(version: string): Promise<string> {
    ...
    const destination = getDownloadedMappingJarPath(version, 'intermediary');
